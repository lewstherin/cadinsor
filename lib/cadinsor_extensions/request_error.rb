...
Module CadinsorExtensions
  ...
  def cadinsor_validate_key(api_key)
    raise CadinsorExtensions::RequestError.new "Invalid Request. Key not present in request." if api_key.to_s == ""
    key = Cadinsor::ApiKey.find_by_key(api_key)
    raise CadinsorExtensions::RequestError.new "Invalid Request. Key is not valid." if !key
    raise CadinsorExtensions::RequestError.new "Invalid Request. Key has expired. Please use a new key." if key.expired?
  end
  ...
  def cadinsor_validate_app(app_id)
    raise CadinsorExtensions::RequestError.new "Invalid Request. Client App id is not present in request." if app_id.to_s == ""
    app = Cadinsor::ClientAPP.find_by_id(app_id)
    raise CadinsorExtensions::RequestError.new "Invalid Request. Client App is not valid." if !app
  end

  def cadinsor_validate_signature(params, signature, app_id_param,name, signature_param_name)
    raise CadinsorExtensions::RequestError.new "Invalid Request. Request signature is not present." if signature.to_s == ""
    request_string = ""
    ...
    raise CadinsorExtensions::RequestError.new "Invalid Request. Request Signature is not valid." if signature != request_hash
  end
end